  ...
  return (
    <div className="w-full h-full flex flex-col bg-white rounded-2xl shadow-lg overflow-hidden">
      ...
      <div className="relative aspect-video bg-gradient-to-br from-gray-100 to-gray-200 flex-shrink-0">
        ...
        >
          <div className="w-16 h-16 rounded-full bg-red-600 hover:bg-red-700 transition-all flex items-center justify-center shadow-xl group-hover:scale-110">
            <Play className="w-9 h-9 text-white ml-1" fill="white" />
          </div>
        </div>
      </div>

      {/* Content Section - Fixed height with overflow */}
      <div className="p-8 flex flex-col flex-1">
        {/* Program Name */}
        <h4 className="text-base font-medium text-purple-500 italic mb-1">
          {testimonial.program}
        </h4>

        {/* Cohort */}
        {testimonial.cohort && (
          <p className="text-sm text-gray-500 mb-6">{testimonial.cohort}</p>
        )}

        {/* Quote - Fixed height with overflow */}
        <blockquote className="text-sm text-gray-700 leading-relaxed flex-1 overflow-hidden line-clamp-6">
          {testimonial.quote}
        </blockquote>
      </div>
    </div>
